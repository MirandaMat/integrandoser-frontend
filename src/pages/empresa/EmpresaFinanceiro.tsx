...
const EmpresaFinanceiro = () => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [statementRes, invoicesRes, annualConsumptionRes] = await Promise.all([
                fetch(`${apiUrl}/api/finance/my-statement/company`, { headers: { 'Authorization': `Bearer ${token}` } }),
                fetch(`${apiUrl}/api/finance/my-invoices`, { headers: { 'Authorization': `Bearer ${token}` } }),
                fetch(`${apiUrl}/api/finance/company/annual-consumption`, { headers: { 'Authorization': `Bearer ${token}` } })
            ]);
    ...
    const handleDownloadInvoice = async (invoiceId: number) => {
        try {
            const response = await fetch(`${apiUrl}/api/finance/invoices/${invoiceId}/download`, {
    ...
    const handleDownloadHistory = async () => {
        try {
            const response = await fetch(`${apiUrl}/api/finance/company/statement/download`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
