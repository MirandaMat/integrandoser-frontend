...
function HomePage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        // 1. ATUALIZE A CHAMADA AXIOS AQUI
    ...
    <>
      ...
      <section id="home" className="hero-section">
        <video 
            ...
            playsInline
        >
          <source src={homeContent.hero_video_url ? `${apiUrl}/${homeContent.hero_video_url}` : "/assets/video_hero.mov"} type="video/mp4" />
        ...
        <Reveal>
          <div className="about-essence-container">
            <div className="about-essence-logo">
                <img 
                  src={aboutContent.logo_url ? `${apiUrl}/${aboutContent.logo_url}` : "/Logo.png"} 
          ...
          <div className="partners-container">
            <div className="partners-slider">
              {aboutContent.partner_logos?.map((logoUrl, index) => (
                <img key={index} src={`${apiUrl}/${logoUrl}`} alt={`Logo Parceiro ${index + 1}`} className="partner-logo-item" />
              ))}
              {/* Duplicamos para o efeito de slider infinito, se houver logos */}
              {aboutContent.partner_logos?.map((logoUrl, index) => (
                <img key={`clone-${index}`} src={`${apiUrl}/${logoUrl}`} alt={`Logo Parceiro ${index + 1}`} className="partner-logo-item" />
              ))}
            </div>
          </div>
        </Reveal>
                ...
                {homeContent.tpt_media_url && (
                  homeContent.tpt_media_type === 'video' ? (
                    <video className="tpt-video" autoPlay loop muted playsInline>
                      <source src={`${apiUrl}/${homeContent.tpt_media_url}`} type="video/mp4" />
                      Seu navegador não suporta o elemento de vídeo.
                    </video>
                  ) : (
                    <img src={`${apiUrl}/${homeContent.tpt_media_url}`} alt="Terapia para todos" className="tpt-video" />
                  )
                )}
              </Reveal>
            </div>
        ...
        <Reveal delay={0.2}> 
          <div className="founder-card">
              <div className="founder-image-container">
                  <img 
                    src={founderContent.image_url ? `${apiUrl}/${founderContent.image_url}` : "/assets/terapeutas/fernanda_oliveira.jpeg"} 
