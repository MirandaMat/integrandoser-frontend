        ...
        if (isOpen) {
            if (isEditMode && userToEdit) {
                ...
                const existingImageUrl = userToEdit.profile?.imagem_url 
                    ? `${apiUrl}/${userToEdit.profile.imagem_url}` 
...
const UsersAdm = () => {
    ...
    const handleSaveUser = async (formData, userId) => {
        ...
        const url = isEditMode ? `${apiUrl}/api/users/${userId}` : `${apiUrl}/api/users`;
                          ...
                          <tr key={u.id}>
                            <td>
                              <div className="user-name-cell" onClick={() => handleOpenPreviewModal(u)}>
                                  <img src={u.imagem_url ? `${apiUrl}/${u.imagem_url}` : '/assets/default-avatar.png'} alt="avatar" className="user-avatar" />
