...
const AdminAgenda = () => {
    ...
    const apiAgenda = useMemo(() => axios.create({ baseURL: `${apiUrl}/api/agenda`, headers: { 'Authorization': `Bearer ${token}` } }), [token]);
    const apiScheduling = useMemo(() => axios.create({ baseURL: `${apiUrl}/api/scheduling`, headers: { 'Authorization': `Bearer ${token}` } }), [token]);
                    ...
                    <div className="future-appointments-list">
                        {futureAppointments.length > 0 ? futureAppointments.map((app: any) => (
                            <div key={app.id} className="future-appointment-item">
                                <img src={app.professional_photo ? `${apiUrl}/${app.professional_photo}` : '/assets/default-avatar.png'} alt={app.professional_name} />
                            ...
                            <div key={app.id} className={`admin-appointment-card ${app.is_pending_review ? 'pending-review' : ''}`}>
                                ...
                                <div className="card-participants">
                                    <div className="participant">
                                        <img src={app.professional_photo ? `${apiUrl}1/${app.professional_photo}` : '/assets/default-avatar.png'} alt={app.professional_name} />
                                        <div className="participant-info"><span>Profissional</span><strong>{app.professional_name}</strong></div>
                                    </div>
                                    <div className="participant">
                                        <img src={app.patient_photo ? `${apiUrl}/${app.patient_photo}` : '/assets/default-avatar.png'} alt={app.patient_name} />
