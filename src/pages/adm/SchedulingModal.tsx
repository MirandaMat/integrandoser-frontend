...
const SchedulingModal = ({ isOpen, onClose, user, api, onConfirm, onReschedule, availableSlots, onNavigateToAgenda }) => {
    ...
    const publicScheduleLink = useMemo(() => {
        ...
        const baseUrl = import.meta.env.PROD
            ? 'https://integrandoser.integrandoser.com.br' // Sua URL de produção
            : 'http://localhost:5173';                   // Sua URL de desenvolvimento

        if (!user) return ''; // Retorna string vazia se o usuário não estiver carregado

        // Monta a URL completa
        return `${baseUrl}/schedule-appointment?triagem_id=${user.id}&type=${user.view}&name=${encodeURIComponent(user.nome)}&email=${encodeURIComponent(user.email)}`;
    }, [user]);
