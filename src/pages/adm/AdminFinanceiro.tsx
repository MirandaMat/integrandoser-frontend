...
const AdminFinanceiro = () => {
  ...
  const fetchData = async () => {
    ...
    try {
        const [kpisRes, transRes, profRevenueRes, pendingInvRes, latestInvoicesRes, monthlyCommissionsRes, allInvoicesRes] = await Promise.all([
            fetch(`${apiUrl}/api/finance/kpis`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/transactions`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/professionals-revenue`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/invoices/pending-approval`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/invoices/latest`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/monthly-commissions`, { headers: { 'Authorization': `Bearer ${token}` } }),
            fetch(`${apiUrl}/api/finance/invoices/all`, { headers: { 'Authorization': `Bearer ${token}` } })
        ]);
  ...
  const approvePayment = async (invoiceId) => {
      const token = localStorage.getItem('token');
      try {
          const response = await fetch(`${apiUrl}/api/finance/invoices/${invoiceId}/status`, {
              method: 'PATCH',
  ...
  const handleGerarRelatorio = async () => {
    const token = localStorage.getItem('token');
    try {
      const response = await fetch(`${apiUrl}/api/finance/report/transactions`, {
