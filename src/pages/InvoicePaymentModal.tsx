...
const InvoicePaymentModal = ({ isOpen, onClose, invoiceId }) => {
    ...
    useEffect(() => {
        if (isOpen && invoiceId) {
            ...
            const fetchDetails = async () => {
                ...
                try {
                    const response = await fetch(`${apiUrl}/api/finance/invoices/${invoiceId}/payment-details`, {
    ...
    const handleFileUpload = async () => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`${apiUrl}/api/finance/invoices/${invoiceId}/upload-receipt`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
                body: formData
            });
