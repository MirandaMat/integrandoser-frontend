...
const ProfissionalDashboard = () => {
    ...
    const api = useMemo(() => axios.create({
        baseURL: `${apiUrl}/api/users`, // Base para a API de usuários/dashboard
        headers: { 'Authorization': `Bearer ${token}` }
    }), [token]);
     const apiAgenda = useMemo(() => axios.create({
        baseURL: `${apiUrl}/api/agenda`,
        headers: { 'Authorization': `Bearer ${token}` }
    }), [token]);
                    ...
                    <div className={`dashboard-widget widget-welcome ${firstPending ? 'pending-review' : ''}`}>
                        ...
                        {firstPending ? (
                            <div className="next-appointment-details pending">
                                <img src={firstPending.patient_photo ? `${apiUrl}/${firstPending.patient_photo}` : '/assets/default-avatar.png'} alt={firstPending.patient_name} />
                            ...
                            <div className="next-appointment-details">
                                <img src={nextAppointment.patient_photo ? `${apiUrl}/${nextAppointment.patient_photo}` : '/assets/default-avatar.png'} alt={nextAppointment.patient_name}/>
                                <div className="next-appointment-info">
