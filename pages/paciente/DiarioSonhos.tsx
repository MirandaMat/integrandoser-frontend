...
const DiarioSonhos = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const res = await fetch('http://localhost:3001/api/dreams/my-dreams', {
    ...
    const handleEditClick = (entry: DreamEntry) => {
        ...
        setImagePreview(entry.image_url ? `http://localhost:3001/${entry.image_url}` : null);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = editingEntry 
            ? `http://localhost:3001/api/dreams/${editingEntry.id}`
    ...
    const handleDelete = async (entryId: number) => {
        if (window.confirm('Tem certeza que deseja apagar este registro?')) {
            try {
                await fetch(`http://localhost:3001/api/dreams/${entryId}`, {
                    method: 'DELETE',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                fetchData();
