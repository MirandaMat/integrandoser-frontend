...
const NotasPaciente = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [notesRes, patientRes] = await Promise.all([
                fetch(`http://localhost:3001/api/notes/${patientId}`, { headers: { 'Authorization': `Bearer ${token}` } }),
    ...
    const handleSaveNote = async (e) => {
        ...
        const url = editingNote ? `http://localhost:3001/api/notes/${editingNote.id}` : 'http://localhost:3001/api/notes';
