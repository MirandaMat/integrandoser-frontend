...
const PacientesProfissional = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [profileRes, associatesRes] = await Promise.all([
                fetch('http://localhost:3001/api/profile/me', { headers: { 'Authorization': `Bearer ${token}` } }),
                fetch('http://localhost:3001/api/users/my-associates', { headers: { 'Authorization': `Bearer ${token}` } })
            ]);
    ...
    const handleSavePatient = async (payload) => {
        setModalError('');
        try {
            const response = await fetch('http://localhost:3001/api/users/professional/create-patient', { //
                method: 'POST', //
                headers: { //
