    ...
    return (
        <div className="note-modal-overlay" onClick={onClose}>
            <div className="note-modal-content" onClick={(e) => e.stopPropagation()}>
                ...
                {entry.image_url && <img src={`http://localhost:3001/${entry.image_url}`} alt={entry.title} className="modal-entry-image" />}
        ...
        const fetchData = async () => {
            ...
            try {
                const [dreamsRes, patientRes] = await Promise.all([
                    fetch(`http://localhost:3001/api/dreams/professional/${patientId}`, { headers: { 'Authorization': `Bearer ${token}` } }),
                    fetch(`http://localhost:3001/api/users/patient-profile/${patientId}`, { headers: { 'Authorization': `Bearer ${token}` } })
                ]);
                ...
                <main className="entries-grid">
                    ...
                    {entries.map(entry => (
                        <article key={entry.id} className="entry-card" onClick={() => setViewingEntry(entry)}>
                            {entry.image_url && <img src={`http://localhost:3001/${entry.image_url}`} alt={entry.title} className="entry-image" />}
