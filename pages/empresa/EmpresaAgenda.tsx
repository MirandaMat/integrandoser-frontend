  ...
  <div className="agenda-item-card">
    ...
    <div className="card-user-content">
      <div className="user-info-item" onClick={() => onUserClick(sessao.patient_user_id)}>
        <img src={sessao.patient_photo ? `http://localhost:3001/${sessao.patient_photo}` : '/assets/default-avatar.png'} alt={sessao.patient_name} className="user-avatar" />
        <div>
      ...
      <div className="user-info-item" onClick={() => onUserClick(sessao.professional_user_id)}>
        <img src={sessao.professional_photo ? `http://localhost:3001/${sessao.professional_photo}` : '/assets/default-avatar.png'} alt={sessao.professional_name} className="user-avatar" />
        <div>
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        const appointmentsPromise = fetch('http://localhost:3001/api/agenda/my-appointments/company', { headers });
        const assignmentsPromise = fetch('http://localhost:3001/api/agenda/company/collaborator-assignments', { headers });
