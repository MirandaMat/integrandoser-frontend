...
const ContentManagement = () => {
    ...
    const apiContent = useMemo(() => axios.create({ baseURL: 'http://localhost:3001/api/content', headers: { 'Authorization': `Bearer ${token}` }}), [token]);
    const apiUsers = useMemo(() => axios.create({ baseURL: 'http://localhost:3001/api/users', headers: { 'Authorization': `Bearer ${token}` }}), [token]);
            ...
            <div className="content-grid">
                {items.map((item: ContentItem) => (
                    <div key={item.id} className="content-card">
                        <img src={`http://localhost:3001/${item.imagem_url || item.image_url || item.photo_url || 'assets/default-placeholder.png'}`} alt={item.title || item.name || item.nome} className="card-image" onError={(e) => { e.currentTarget.src = '/assets/default-placeholder.png'; }} />
